...
export const DealDetailModal: React.FC<DealDetailModalProps> = ({ dealId, isOpen, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'timeline' | 'products' | 'info' | 'playbook'>('timeline');
            ...
            <div className="h-14 border-b border-slate-200 dark:border-white/5 flex items-center px-6 shrink-0">
              <div className="flex gap-6">
                ...
                <button
                  ...
                  className={`text-sm font-bold h-14 border-b-2 transition-colors ${activeTab === 'playbook' ? 'border-primary-500 text-primary-600 dark:text-white' : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-white'}`}
                >
                  Playbook
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30 dark:bg-black/10">
              ...
              {activeTab === 'playbook' && (
                <div className="p-6 animate-in fade-in slide-in-from-bottom-4">
                  <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-4 shadow-sm">
                    <DealChecklist dealId={deal.id} boardId={deal.boardId} stageId={deal.status} />
                  </div>
                </div>
              )}
