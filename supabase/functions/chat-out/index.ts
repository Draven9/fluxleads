...
Deno.serve(async (req: Request) => {
    ...
    const isGroup = session.contact?.source === 'whatsapp_group'
        || (session.provider_id || '').includes('@g.us');

    if (isGroup && contactPhone && !contactPhone.includes('@')) {
        contactPhone = `${contactPhone}@g.us`;
    }

    // Fallback: if contact has no phone, use session.provider_id directly
    if (!contactPhone && session.provider_id) {
        contactPhone = session.provider_id;
    }
